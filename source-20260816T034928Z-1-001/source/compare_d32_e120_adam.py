"""Compare 32d/120-epoch CoxRes-KGA with retained 16d/170-epoch settings."""
...
CANDIDATE = ROOT / "results" / "sensitivity" / "d32_e120_adam" / "predictions" / "all_predictions.csv"
OUTPUT = ROOT / "results" / "sensitivity" / "d32_e120_adam" / "metrics"
...
def main() -> None:
    ...
    for strategy, validation in VALIDATION_LABELS.items():
        ...
        for metric in METRICS:
            ...
            summary.update(
                {
                    f"d16_e170_{metric}": baseline_point[metric],
                    f"d32_e120_{metric}": candidate_point[metric],
    ...
    summary.to_csv(OUTPUT / "d16_e170_vs_d32_e120.csv", index=False)
    pd.DataFrame(replicate_rows).to_csv(
        OUTPUT / "d16_e170_vs_d32_e120_replicates.csv", index=False
    )
