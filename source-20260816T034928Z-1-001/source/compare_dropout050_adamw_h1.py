"""Compare dropout 0.50/AdamW with dropout 0.40/Adam for one-head CoxRes-KGA."""
...
CANDIDATE = ROOT / "results" / "sensitivity" / "dropout050_adamw_h1" / "predictions" / "all_predictions.csv"
OUTPUT = ROOT / "results" / "sensitivity" / "dropout050_adamw_h1" / "metrics"
...
def main() -> None:
    ...
    for strategy, validation in VALIDATION_LABELS.items():
        ...
        for metric in METRICS:
            ...
            summary.update(
                {
                    f"dropout040_adam_{metric}": baseline_point[metric],
                    f"dropout050_adamw_{metric}": candidate_point[metric],
    ...
    summary.to_csv(OUTPUT / "dropout040_adam_vs_dropout050_adamw_h1.csv", index=False)
    pd.DataFrame(replicate_rows).to_csv(
        OUTPUT / "dropout040_adam_vs_dropout050_adamw_h1_replicates.csv",
        index=False,
    )
